...
const { minProfit, winRate, minPnl, APPOLO, providerUrl, nodeProviderUrl, etherscankey, apiUrl } = require("../config/config.js");
...
async function checkSwapWallet(targetAddress, poolAddress) {
  try {
    let swaps2 = await getSwaps(poolAddress, targetAddress, 2)
    let swaps3 = await getSwaps(poolAddress, targetAddress, 3)
